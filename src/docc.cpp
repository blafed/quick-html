...
//
//  my_container implementation
//

uint_ptr my_container::create_font(const char* faceName, int size, int weight, litehtml::font_style italic, unsigned int decoration, litehtml::font_metrics* fm)
{
    Font* font = new Font {
        faceName,
        size,
...
int my_container::text_width(const char* text, uint_ptr hFont)
{
	Font* font = (Font*)hFont;
	return font->size;
}

void my_container::draw_text(uint_ptr hdc, const char* text, uint_ptr hFont, web_color color, const position& pos)
{
...
int my_container::pt_to_px(int pt) const { return pt * 96 / 72; }
int my_container::get_default_font_size() const { return 16; }
const char* my_container::get_default_font_name() const { return "Terminus"; }

void my_container::draw_solid_fill(uint_ptr hdc, const background_layer& layer, const web_color& color)
{
	auto cvs = (canvas*)hdc;
	fill_rect(*cvs, layer.border_box, color);
}

void my_container::draw_borders(uint_ptr hdc, const borders& borders, const position& pos, bool /*root*/)
{
...
void my_container::draw_list_marker(uint_ptr hdc, const list_marker& marker)
{
	auto& cvs = *(canvas*)hdc;
...
string my_container::make_url(const char* src, const char* baseurl)
{
	return (baseurl && *baseurl ? getdir(baseurl) : basedir) + "/" + src;
}

void my_container::import_css(string& text, const string& url, string& baseurl)
{
	baseurl = make_url(url.c_str(), baseurl.c_str());
	text = readfile(baseurl);
}

void my_container::get_client_rect(position& client) const
{
	client = {0, 0, width, height};
}

void my_container::get_media_features(media_features& media) const
{
	position client;
...
void my_container::load_image(const char* src, const char* baseurl, bool /*redraw_on_ready*/)
{
	string url = make_url(src, baseurl);
	images[url] = Bitmap(url);
}

void my_container::get_image_size(const char* src, const char* baseurl, size& sz)
{
	string url = make_url(src, baseurl);
	auto& img = images[url];
...
void my_container::draw_image(uint_ptr hdc, const background_layer& bg, const string& src, const string& base_url)
{
	auto& cvs = *(canvas*)hdc;
...
void my_container::draw_linear_gradient(uint_ptr hdc, const background_layer& layer, const background_layer::linear_gradient& gradient)
{
	draw_gradient(hdc, layer, gradient);
}

void my_container::draw_radial_gradient(uint_ptr hdc, const background_layer& layer, const background_layer::radial_gradient& gradient)
{
	draw_gradient(hdc, layer, gradient);
}

void my_container::draw_conic_gradient(uint_ptr hdc, const background_layer& layer, const background_layer::conic_gradient& gradient)
{
	draw_gradient(hdc, layer, gradient);
}